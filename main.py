...
class Config:
    APP_NAME = "Mundo Dron v9.5 (Anti-Crash)"
...
def main(page: ft.Page):
    ...
    # === ESCUDO ANTI-CRASH PARA ANDROID ===
    try:
        # Le pedimos a Android que nos de SU carpeta de sistema privada de forma dinámica
        ruta_base = os.environ.get("HOME", os.getcwd())
        
        db_path = os.path.join(ruta_base, "mi_base.db")
        backup_dir = os.path.join(ruta_base, "backups")
        ...
        # Si algo falla en la memoria de Android, mostramos una pantalla roja con el error
        error_msg = traceback.format_exc()
        page.add(
            ft.AppBar(title=ft.Text("🛠️ MODO DIAGNÓSTICO"), bgcolor="red900"),
            ft.Container(
                content=ft.Column([
                    ft.Text("💥 ERROR DE PERMISOS DE ANDROID", size=18, weight="bold", color="red"),
                    ft.Text("Mándale una captura de esto a tu analista:", color="white"),
                    ft.Divider(),
                    ft.Text(error_msg, color="amber", selectable=True, size=12),
    ...
    # === SUB-PANTALLA: EDITOR Y PLANTILLAS ===
    ...
    def vista_herramientas():
        ...
        def ejecutar_backup(e):
            try:
                ...
                destino = os.path.join(ruta_base, "backups", f"backup_{timestamp}.db")
                shutil.copy(db_path, destino)
                mostrar_alerta("Backup Exitoso", f"Archivo guardado en la memoria interna de la app.")
        ...
        def ejecutar_csv(e):
            try:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                path = os.path.join(ruta_base, "exports", f"inventario_{timestamp}.csv")
                ...
                mostrar_alerta("CSV Creado", f"Excel generado internamente.")
            except Exception as ex:
